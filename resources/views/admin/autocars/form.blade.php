    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <form action="{{ $autocar->id ? route('admin.autocars.update', $autocar) : route('admin.autocars.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="matricule" value="{{ old('matricule', $autocar->matricule) }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" required />
                @error('matricule')
            ...
            <div>
                ...
                <input type="number" name="capacite" value="{{ old('capacite', $autocar->capacite) }}" min="1" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" required />
                @error('capacite')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="societe_id" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" required>
            ...
            <div>
                ...
                <select name="type" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" required>
            ...
            <div class="flex gap-3 pt-4 border-t border-slate-200">
                ...
                <button type="submit" class="rounded-2xl bg-blue-600 px-6 py-3 text-white font-semibold hover:bg-blue-700 transition">{{ $autocar->id ? 'Mettre à jour' : 'Créer' }}</button>
            </div>
        </form>
    </div>
</div>
