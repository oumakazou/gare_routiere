    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <form action="{{ $societe->id ? route('admin.societes.update', $societe) : route('admin.societes.store') }}" method="POST" class="space-y-6 max-w-md">
            ...
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom', $societe->nom) }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" required />
                @error('nom')
            ...
            <div>
                ...
                <input type="text" name="contact" value="{{ old('contact', $societe->contact) }}" placeholder="Téléphone ou email" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
                @error('contact')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-3 pt-4 border-t">
                ...
                <button type="submit" class="rounded-2xl bg-blue-600 px-6 py-3 text-white font-semibold hover:bg-blue-700">{{ $societe->id ? 'Mettre à jour' : 'Créer' }}</button>
            </div>
        </form>
    </div>
</div>
