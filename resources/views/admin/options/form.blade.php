    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <form action="{{ $option->id ? route('admin.options.update', $option) : route('admin.options.store') }}" method="POST" class="space-y-6 max-w-md">
            ...
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom', $option->nom) }}" placeholder="Ex: VIP, Confort..." class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" required />
                @error('nom')
            ...
            <div class="flex gap-3 pt-4 border-t">
                ...
                <button type="submit" class="rounded-2xl bg-blue-600 px-6 py-3 text-white font-semibold hover:bg-blue-700">{{ $option->id ? 'Mettre à jour' : 'Créer' }}</button>
            </div>
        </form>
    </div>
</div>
