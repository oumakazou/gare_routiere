    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <form action="{{ $ville->id ? route('admin.villes.update', $ville) : route('admin.villes.store') }}" method="POST" class="space-y-6 max-w-md">
            ...
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom', $ville->nom) }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:border-red-500 focus:ring-2 focus:ring-red-200" required />
                @error('nom')
            ...
            <div class="flex gap-3 pt-4 border-t">
                ...
                <button type="submit" class="rounded-2xl bg-red-600 px-6 py-3 text-white font-semibold hover:bg-red-700">{{ $ville->id ? 'Mettre à jour' : 'Créer' }}</button>
            </div>
        </form>
    </div>
</div>
