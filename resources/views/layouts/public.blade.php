...
<body class="min-h-screen bg-slate-100 text-slate-900">
    <header class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
            <a href="{{ route('voyages.index') }}" class="text-xl font-bold text-blue-700">Gare Routiere</a>
    ...
    <main>
        @if(session('success'))
            <div class="mx-auto mt-6 max-w-7xl rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700 sm:px-6 lg:px-8">
                {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
